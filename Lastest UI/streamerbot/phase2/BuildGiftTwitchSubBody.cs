// Streamer.bot inline C# — Twitch Gift Subscription (single) → /api/donation/gift-membership (R6a).
// Credits recipientUserName. Writes donation_gift_body.json for curl.
...
public class CPHInline
{
    ...
    {
        string user = "";
        CPH.TryGetArg("recipientUserName", out user);
        ...
        WriteGiftJson(user, tier, isSub, isMember);
        return true;
    }

    void WriteGiftJson(string user, string tier, bool isSub, bool isMember)
    {
        ...
        sb.Append("}");
        try { File.WriteAllText(BODY_FILE, sb.ToString()); } catch (Exception ex) { CPH.LogInfo("BuildGiftTwitchSubBody: " + ex.Message); }
    }
