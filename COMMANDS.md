...
| **!topsummoner** | **Sprint** leader this Bestiary level among viewers who haven’t crowned yet this stream (resets on level-up and Halls loops; not the 2× heat crown) |
...
| **!givepoints** \<amount\> \<target\> | Free | Transfer **chat points** to another viewer. Works on YouTube without `@`: `!givepoints 50 bob` (Twitch also accepts `@bob`). Spends chat points first, then donor points if needed. Recipient cannot go over the live chat cap; overflow is not awarded. |
