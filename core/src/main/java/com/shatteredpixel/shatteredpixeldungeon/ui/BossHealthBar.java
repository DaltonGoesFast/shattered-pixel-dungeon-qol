...
public class BossHealthBar extends Component {
	...
	public BossHealthBar() {
		super();
		// Keep active=true so we always receive update() and can show bar when user targets
		active = true;
	...
	/** Returns the Mob to display: the assigned boss, or the current target if bossBarAllEnemies. */
	...
	public void update() {
		...
		int incomingDOT = toShow.incomingDOT();
		int max = toShow.HT;
	...
	private void setupDisplayMob(Mob mob) {
		if (large && mob.sprite != null) {
			if (skull != null) {
				remove(skull);
				// only destroy default icon, not a mob's CharSprite
				if (!(skull instanceof CharSprite)) skull.destroy();
			}
			skull = mob.sprite();
			add(skull);
		}
			...
			if (forceSpriteRefresh && instance.large){
				ShatteredPixelDungeon.runOnRenderThread(new Callback() {
					...
					public void call() {
						...
						if (!(instance.skull instanceof CharSprite)) instance.skull.destroy();
						instance.skull = boss.sprite();
						instance.add(instance.skull);
						instance.currentDisplayMob = boss;
						instance.layout();
					}
				});
			}
